...
#         t_y = []
        ...
        while 1:
            ...
            with torch.no_grad():
                ...
                # delay,_ = divmod (p_mean+(t3-t1) , 0.0333)
                delay,_ = divmod (prev_inf_time+(t3-t1) , 0.0333)
                print(int(delay))
                result, buffer = model(frame, buffer=buffer, mode='on_pipe',delay=int(delay))
                #end
                # result, buffer = model(frame, buffer=buffer, mode='on_pipe',delay=0)
            ...
            #     p_total = p_total - t_o
            r_time = t2 - t3
            # p_quene.append(r_time)
            # p_total = p_total + r_time
            # p_frames = len(p_quene)
            # p_mean = p_total / p_frames
            # print(p_quene)
            # print(p_frames)
            # print(p_total)
            # print(p_mean)

            if r_time/r_time_1<1.00:
                prev_inf_time = r_time
            else:
                prev_inf_time = r_time_1
            ...
            #modify
            preprocess.append(t3 - t1)
            inference_1.append(t2 - t3)
            #end
